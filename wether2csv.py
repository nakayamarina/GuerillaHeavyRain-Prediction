...
#            2015年1年分の時間ごとの気温、湿度、風向・風量、雨量のcsvファイル
...
#雨量
rain_data = pd.read_csv("./dataset_kochi/rain.csv", encoding = 'shift-jis', skiprows = 4)

rain = rain_data.iloc[:, 0:2]
rain.columns = ['date', 'rain']

#気温
temp_data = pd.read_csv("./dataset_kochi/temp.csv", encoding = 'shift-jis', skiprows = 3)

temp_data.columns = ['date', 'temp']
temp = temp_data

#風向・風量
wind_data = pd.read_csv("./dataset_kochi/wind.csv", encoding = 'shift-jis', skiprows = 4)

wind = wind_data.iloc[:, 0:2]
wind.columns = ['date', 'wind']

#湿度
humidity_data = pd.read_csv("./dataset_kochi/humidity.csv", encoding = 'shift-jis', skiprows = 3)
